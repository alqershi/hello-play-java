...
public class Application extends Controller {
	...
	public static WebSocket<JsonNode> buzz() {
		return new WebSocket<JsonNode>() {
			...
			public void onReady(play.mvc.WebSocket.In<JsonNode> in,
				...
				EventPublisher.ref.tell(new NewConnectionEvent(uuid, out),
						ActorRef.noSender());

				in.onClose(new Callback0() {
					@Override
					public void invoke() throws Throwable {
						EventPublisher.ref.tell(new CloseConnectionEvent(uuid),
								ActorRef.noSender());
					}
				});
			}
	...
	public static Promise<Result> submitProposal() {
		...
		if (submittedForm.hasErrors()) {
			return Promise.<Result> pure(ok(views.html.newProposal
					.render(submittedForm)));
		} else {
			final Proposal proposal = submittedForm.get();
			Promise<Result> r = proposal.asyncSave().map(
